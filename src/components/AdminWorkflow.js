...
const AdminWorkflow = () => {
  ...
  useEffect(() => {
    ...
    setCandidates(data);
    setFilteredCandidates(data); // Set filteredCandidates to initial candidates data

    // Calculate domain data for pie chart
    const domainCount = data.reduce((acc, candidate) => {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      const filtered = candidates.filter((candidate) => {
        const candidateDate = new Date(candidate.timestamp);
        ...
        const matchesDateRange =
          (!from || candidateDate >= from) && (!to || candidateDate <= to);
  ...
  const handleShortlist = (id) => {
    ...
    setFilteredCandidates(updatedCandidates); // Update filteredCandidates as well
    localStorage.setItem("userData", JSON.stringify(updatedCandidates));
  };
  ...
  const handleDeleteAll = () => {
    setCandidates([]);
    setFilteredCandidates([]); // Clear filteredCandidates as well
  ...
  return (
    <div className="admin-workflow">
          ...
          {filteredCandidates.map((candidate) => (
            <tr key={candidate.id}>
              <td>
                <input
                  type="checkbox"
                  checked={candidate.shortlisted || false}
                  onChange={() => handleShortlist(candidate.id)}
                />
              </td>
      ...
      <div class="last-row">
        <Link to="/admin/shortlisted">View Shortlisted Candidates</Link>
